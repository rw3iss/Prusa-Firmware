...
#define BED_HYSTERESIS \
    2  // only disable heating if T>target+BED_HYSTERESIS and enable heating if T>target-BED_HYSTERESIS
#endif
...
#define EXTRUDER_RUNOUT_SPEED 1000.  // extrusion speed
#define EXTRUDER_RUNOUT_EXTRUDE 80

// These defines help to calibrate the AD595 sensor in case you get wrong temperature measurements.
// The measured temperature is defined as "actualTemp = (measuredTemp * TEMP_SENSOR_AD595_GAIN) +
...
// Only a few motherboards support this, like RAMPS, which have dual extruder support (the 2nd, often unused, extruder
// driver is used to control the 2nd Z axis stepper motor). The pins are currently only defined for a RAMPS
// motherboards. On a RAMPS (or other 5 driver) motherboard, using this feature will limit you to using 1 extruder.
...
// #define QUICK_HOME  //if this is defined, if both x and y are to be homed, a diagonal move will be performed
// initially.

#define MAX_STEP_FREQUENCY \
    40000  // Max step frequency for Ultimaker (5000 pps / half step). Toshiba steppers are 4x slower, but Prusa3D does
           // not use those.
...
#define SDCARD_RATHERRECENTFIRST  // reverse file order of sd card menu display. Its sorted practically after the file
                                  // system block order.
// if a file is deleted, it frees a block. hence, the order is not purely chronological. To still have auto0.g
// accessible, there is again the option to do that. using:
...
// Enabe this option to get a pretty message whenever the endstop gets hit (as in the position at which the endstop got
// triggered)
// #define VERBOSE_CHECK_HIT_ENDSTOPS

// Enable the option to stop SD printing when hitting and endstops, needs to be enabled from the LCD menu when this
// option is enabled.
...
// 1st byte: the command source (CMDBUFFER_CURRENT_TYPE_USB, CMDBUFFER_CURRENT_TYPE_SDCARD, CMDBUFFER_CURRENT_TYPE_UI or
// CMDBUFFER_CURRENT_TYPE_CHAINED) 2nd and 3rd byte (LSB first) contains a 16bit length of a command including its
// preceding comments.
#define CMDHDRSIZE 3
...
#define RETRACT_RECOVER_LENGTH_SWAP \
    0  // default additional swap recover length (mm, added to retract length when recovering from extruder change)
