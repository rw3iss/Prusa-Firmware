...
// #define DEFAULT_Y_OFFSET    4.f // Default distance of Y_MIN_POS point from endstop, when the printer is not
// calibrated.
/**
...
#define DEFAULT_MAX_FEEDRATE \
    { 200, 200, 12, 120 }  // (mm/sec)   max feedrate (M203)
#define DEFAULT_MAX_FEEDRATE_SILENT \
    { 100, 100, 12, 120 }  // (mm/sec)   max feedrate (M203), silent mode

#define DEFAULT_MAX_ACCELERATION \
    { 1000, 1000, 200, 5000 }  // (mm/sec^2) max acceleration (M201)
#define DEFAULT_MAX_ACCELERATION_SILENT \
    { 960, 960, 200, 5000 }  // (mm/sec^2) max acceleration (M201), silent mode
...
#define MANUAL_FEEDRATE \
    { 2700, 2700, 1000, 100 }  // set the speeds for manual moves (mm/min)
...
// temperature inside the case is around 31C for ambient temperature 25C, when the printer is powered on long time and
// idle the real limit is 15C (same as MINTEMP limit), this is because 15C is end of scale for both used thermistors
// (bed, heater)
...
#define TMC2130_PWM_CLK \
    (2 * TMC2130_FCLK / TMC2130_PWM_DIV)  // PWM frequency (23.4kHz, 35.1kHz, 46.9kHz, 58.5kHz for 12MHz fclk)

#define TMC2130_TPWMTHRS \
    0  // TPWMTHRS - Sets the switching speed threshold based on TSTEP from stealthChop to spreadCycle mode
...
// new settings is possible for vsense = 1, running current value > 31 set vsense to zero and shift both currents by 1
// bit right (Z axis only)
#define TMC2130_CURRENTS_H \
    { 16, 20, 35, 30 }            // default holding currents for all axes
#define TMC2130_CURRENTS_FARM 36  // E 805 mA peak for ECool/farm mode
#define TMC2130_CURRENTS_R \
    { 16, 20, 35, 30 }  // default running currents for all axes
#define TMC2130_CURRENTS_R_HOME \
    { 8, 10, 20, 18 }  // homing running currents for all axes
...
#define FILAMENTCHANGE_FIRSTFEED \
    70  // E distance in mm for fast filament loading sequence used used in filament change (M600)
#define FILAMENTCHANGE_FINALFEED \
    25  // E distance in mm for slow filament loading sequence used used in filament change (M600) and filament load
        // (M701)
#define FILAMENTCHANGE_RECFEED 5

#define FILAMENTCHANGE_XYFEED 50
#define FILAMENTCHANGE_EFEED_FIRST \
    20  // feedrate in mm/s for fast filament loading sequence used in filament change (M600)
#define FILAMENTCHANGE_EFEED_FINAL \
    3.3f  // feedrate in mm/s for slow filament loading sequence used in filament change (M600) and filament load (M701)
...
// After the loading sequence and after a print is canceled, the filament is retracted to get it out of the heat zone of
// the nozzle. Then a small extrusion is performed to make sure the filament is close enough for the next print without
// oozing. #define COMMUNITY_PREVENT_OOZE
...
#define PLA2_PREHEAT_HOTEND_TEMP 210
#define PLA2_PREHEAT_HPB_TEMP 60

#define CLEAN_PREHEAT_HOTEND_TEMP 275
#define CLEAN_PREHEAT_HPB_TEMP 0
...
#define PINDA_HEAT_T 60  // time in s
...
// If power panic occured, and the current temperature is higher then target temperature before interrupt minus this
// offset, print will be recovered automatically.
...
/* A value of 1 or less for N_ARC_CORRECTION will trigger the use of Sin and Cos for every arc, which will improve
...
#define DEFAULT_MIN_ARC_SEGMENTS \
    20  // The enforced minimum segments in a full circle of the same radius.  Set to 0 to disable
