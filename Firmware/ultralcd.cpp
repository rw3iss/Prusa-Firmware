...
static bool extraPurgeNeeded =
    false;  ///< lcd_commands - detect if extra purge after MMU-toolchange is necessary or not
...
// static void lcd_menu_show_sensors_state();      // NOT static due to using inside "Marlin_main" module
// ("manage_inactivity()")
...
static uint8_t lcd_selftest_screen(TestScreen screen, uint8_t _progress, uint8_t _progress_scale, bool _clear,
                                   uint16_t _delay);
static void lcd_selftest_screen_step(const uint8_t _row, const uint8_t _col, const uint8_t _state, const char *_name,
...
#define MENU_ITEM_SDDIR(str_fn, str_fnl)  \
    do {                                  \
        menu_item_sddir(str_fn, str_fnl); \
    } while (0)
#define MENU_ITEM_SDFILE(str_fn, str_fnl)  \
    do {                                   \
        menu_item_sdfile(str_fn, str_fnl); \
    } while (0)
...
uint8_t lcdui_print_extruder(void) {
    ...
    if (MMU2::mmu2.get_current_tool() == MMU2::mmu2.get_tool_change_tool()) {
        lcd_putc('F');
        lcd_putc(MMU2::mmu2.get_current_tool() == (uint8_t)MMU2::FILAMENT_UNKNOWN
                     ? '?'
                     : MMU2::mmu2.get_current_tool() + '1');
        chars += 2;
    } else {
        lcd_putc(MMU2::mmu2.get_current_tool() == (uint8_t)MMU2::FILAMENT_UNKNOWN
                     ? '?'
                     : MMU2::mmu2.get_current_tool() + '1');
        lcd_putc('>');
        lcd_putc(MMU2::mmu2.get_tool_change_tool() == (uint8_t)MMU2::FILAMENT_UNKNOWN
                     ? '?'
                     : MMU2::mmu2.get_tool_change_tool() + '1');
        chars += 3;
    }
    return chars;
}
...
static void lcdui_print_farm(void) { lcd_printf_P(_N(" FRM ")); }
...
void lcdui_print_time(void) {
    ...
    if (printer_active()) {
        ...
        if (SilentModeMenu != SILENT_MODE_OFF) {
            if (print_time_remaining_silent != PRINT_TIME_REMAINING_INIT) print_tr = print_time_remaining_silent;
            // #ifdef CLOCK_INTERVAL_TIME
            if (print_time_to_change_silent != PRINT_TIME_REMAINING_INIT) print_tc = print_time_to_change_silent;
            // #endif //CLOCK_INTERVAL_TIME
        } else {
#endif  // TMC2130
            if (print_time_remaining_normal != PRINT_TIME_REMAINING_INIT) print_tr = print_time_remaining_normal;
            // #ifdef CLOCK_INTERVAL_TIME
            if (print_time_to_change_normal != PRINT_TIME_REMAINING_INIT) print_tc = print_time_to_change_normal;
// #endif //CLOCK_INTERVAL_TIME
#ifdef TMC2130
        }
#endif  // TMC2130
        ...
        // #ifdef CLOCK_INTERVAL_TIME
        if (clock_interval == CLOCK_INTERVAL_TIME * 2) clock_interval = 0;
        ...
        chars =
            lcd_printf_P(PSTR("Nd %4.2f "), (float)eeprom_read_word((uint16_t *)EEPROM_NOZZLE_DIAMETER_uM) / 1000.0);
#else
...
void lcdui_print_status_line(void) {
    ...
    if (heating_status != HeatingStatus::NO_HEATING) {  // If heating flag, show progress of heating
        ...
        if (custom_message_type != CustomMsg::Status && lcd_status_message_timeout.running() &&
        ...
        switch (custom_message_type) {
            ...
            case CustomMsg::MMUProgress:      // MMU Progress Codes
            {
                ...
                const uint8_t padding =
                    lcd_print_pad(&lcd_status_message[lcd_status_message_idx], LCD_WIDTH - lcd_status_message_idx);
...
// Main status screen. It's up to the implementation specific part to show what is needed. As this is very display
// dependent
void lcd_status_screen()  // NOT static due to using inside "Marlin_main" module ("manage_inactivity()")
{
    ...
    if (lcd_status_update_delay)
        ...
        if (lcd_commands_type != LcdCommands::Idle) lcd_commands();
    }
...
void lcd_commands() {
        ...
        if (!blocks_queued() && cmd_buffer_empty() && !saved_printing) {
            ...
            switch (lcd_commands_step) {
                ...
                case 1:
                    ...
                    lcd_commands_type = LcdCommands::Idle;
                    SetPrinterState(PrinterState::Idle);
                    if (eeprom_read_byte((uint8_t *)EEPROM_WIZARD_ACTIVE)) lcd_wizard(WizState::RepeatLay1Cal);
                    break;
            }
        }
    }
    ...
    if (lcd_commands_type == LcdCommands::ThermalModel && cmd_buffer_empty()) {
        switch (lcd_commands_step) {
            ...
            case 2:
                if (thermal_model_autotune_result()) enquecommand_P(MSG_M500);
                lcd_commands_step = 1;
                break;
        ...
        if (!blocks_queued() && cmd_buffer_empty() && !saved_printing) {
            ...
            switch (lcd_commands_step) {
                ...
                case 2:
                    ...
                    lcd_update_enabled = false;  // hack to avoid lcd_update recursion.
                    if (lcd_show_fullscreen_message_yes_no_and_wait_P(_T(MSG_NOZZLE_CNG_CHANGED), false) ==
...
void lcd_pause_usb_print() { SERIAL_PROTOCOLLNRPGM(MSG_OCTOPRINT_ASK_PAUSE); }
...
static uint16_t __attribute__((noinline)) clamp999(uint16_t v) { return v > 999 ? 999 : v; }
...
static void lcd_menu_fails_stats_mmu_print() {
    ...
    lcd_printf_P(PSTR("%S\n"
                      " %-16.16S%-3d\n"
                      " %-16.16S%-3d"),
                 _T(MSG_LAST_PRINT_FAILURES), _T(MSG_MMU_FAILS), clamp999(eeprom_read_byte((uint8_t *)EEPROM_MMU_FAIL)),
...
static void lcd_menu_fails_stats_mmu_total() {
    ...
    lcd_printf_P(PSTR("%S\n"
                      ...
                      " %-16.16S%-3d\n"
                      " %-16.16S%-3d"),
                 _T(MSG_TOTAL_FAILURES), _T(MSG_MMU_FAILS), clamp999(eeprom_read_word((uint16_t *)EEPROM_MMU_FAIL_TOT)),
...
#if defined(TMC2130) && defined(FILAMENT_SENSOR)
static const char failStatsFmt[] PROGMEM =
    "%S\n"
    " %-16.16S%-3d\n"
    " %-16.16S%-3d\n"
    " %-7.7SX %-3d  Y %-3d";
...
static void lcd_menu_fails_stats_total() {
    lcd_timeoutToStatus.stop();  // infinite timeout
    lcd_home();
    lcd_printf_P(failStatsFmt, _T(MSG_TOTAL_FAILURES), _T(MSG_POWER_FAILURES),
                 clamp999(eeprom_read_word((uint16_t *)EEPROM_POWER_COUNT_TOT)), _T(MSG_FIL_RUNOUTS),
                 clamp999(eeprom_read_word((uint16_t *)EEPROM_FERROR_COUNT_TOT)), _T(MSG_CRASH),
...
static void lcd_menu_fails_stats_print() {
    ...
    lcd_home();
    lcd_printf_P(failStatsFmt, _T(MSG_LAST_PRINT_FAILURES), _T(MSG_POWER_FAILURES), power, _T(MSG_FIL_RUNOUTS), filam,
...
static void lcd_menu_fails_stats() {
    ...
    lcd_home();
    lcd_printf_P(failStatsFmt, _T(MSG_LAST_PRINT_FAILURES), _T(MSG_FIL_RUNOUTS), filamentLast, _T(MSG_TOTAL_FAILURES),
...
//! ----------------------
//! @endcode
//! @todo Positioning of the messages and values on LCD aren't fixed to their exact place. This causes issues with
//! translations.
...
#endif  // defined (VOLT_BED_PIN) || defined (VOLT_PWR_PIN) || (defined(FILAMENT_SENSOR) && (FILAMENT_SENSOR_TYPE ==
        // FSENSOR_IR_ANALOG))
...
static void lcd_menu_belt_status() {
    ...
    lcd_printf_P(PSTR("%S\n"
                      " X %d\n"
                      " Y %d"),
                 _T(MSG_BELT_STATUS), eeprom_read_word((uint16_t *)(EEPROM_BELTSTATUS_X)),
...
static void lcd_menu_test_save() { stop_and_save_print_to_ram(10, -0.8); }

static void lcd_menu_test_restore() { restore_print_from_ram_and_continue(0.8); }
#endif  // RESUME_DEBUG
...
static void lcd_serial_dump() { serial_dump_and_reset(dump_crash_reason::manual); }
...
void TestPullupCrash() { PORTF |= 0x01; }
...
#endif  // MENU_DUMP
#ifdef MENU_SERIAL_DUMP
    if (emergency_serial_dump) MENU_ITEM_FUNCTION_P(_n("Dump to serial"), lcd_serial_dump);
#endif
#ifdef DEBUG_BUILD
...
bool shouldPreheatOnlyNozzle() {
    ...
    if (eeprom_setting != 0) return false;
...
static void mFilamentPrompt() {
    ...
    if (lcd_clicked()
        ...
        || (((eFilamentAction == FilamentAction::Load) || (eFilamentAction == FilamentAction::AutoLoad)) &&
...
static void setFilamentAction(FilamentAction action) { eFilamentAction = action; }
...
static void __attribute__((noinline)) mFilamentResetMenuStack() { menu_back(bFilamentPreheatState ? 1 : 2); }
...

    {
        ...
        if (action == FilamentAction::Preheat || action == FilamentAction::Lay1Cal) {
            ...
            if (action == FilamentAction::Lay1Cal) {
                ...
                if (eeprom_read_byte((uint8_t *)EEPROM_WIZARD_ACTIVE)) lcd_wizard(WizState::LoadFilHot);
            }
            return;
        }
    }
        ...
        switch (eFilamentAction) {
            ...
            case FilamentAction::UnLoad:
                if (bFilamentWaitingFlag)
                    menu_submenu(mFilamentPrompt, true);
                else {
...
static void mFilamentItem_PLA2() {
    bFilamentPreheatState = false;
    mFilamentItem(PLA2_PREHEAT_HOTEND_TEMP, PLA2_PREHEAT_HPB_TEMP);
}

static void mFilamentItem_Clean() {
    bFilamentPreheatState = false;
    mFilamentItem(CLEAN_PREHEAT_HOTEND_TEMP, CLEAN_PREHEAT_HPB_TEMP);
}
...
void lcd_generic_preheat_menu() {
    MENU_BEGIN();
    if (!eeprom_read_byte((uint8_t *)EEPROM_WIZARD_ACTIVE)) {
        ON_MENU_LEAVE(clearFilamentAction(););
        MENU_ITEM_BACK_P(_T(eFilamentAction == FilamentAction::Lay1Cal ? MSG_BACK : MSG_MAIN));
    }
    if (farm_mode) {
        MENU_ITEM_FUNCTION_P(
            PSTR("farm   -  " STRINGIFY(FARM_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(FARM_PREHEAT_HPB_TEMP)),
            mFilamentItem_farm);
        ...
        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("PLA  -  " STRINGIFY(PLA_PREHEAT_HOTEND_TEMP))
                : PSTR("PLA  -  " STRINGIFY(PLA_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PLA_PREHEAT_HPB_TEMP)),
            mFilamentItem_PLA);

        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("PLA2  -  " STRINGIFY(PLA2_PREHEAT_HOTEND_TEMP))
                : PSTR("PLA2  -  " STRINGIFY(PLA2_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PLA2_PREHEAT_HPB_TEMP)),
            mFilamentItem_PLA2);

        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("PET  -  " STRINGIFY(PET_PREHEAT_HOTEND_TEMP))
                : PSTR("PET  -  " STRINGIFY(PET_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PET_PREHEAT_HPB_TEMP)),
            mFilamentItem_PET);
        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("ASA  -  " STRINGIFY(ASA_PREHEAT_HOTEND_TEMP))
                : PSTR("ASA  -  " STRINGIFY(ASA_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(ASA_PREHEAT_HPB_TEMP)),
            mFilamentItem_ASA);
        MENU_ITEM_SUBMENU_P(bPreheatOnlyNozzle
                                ? PSTR("PC   -  " STRINGIFY(PC_PREHEAT_HOTEND_TEMP))
                                : PSTR("PC   -  " STRINGIFY(PC_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PC_PREHEAT_HPB_TEMP)),
                            mFilamentItem_PC);
        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("PVB  -  " STRINGIFY(PVB_PREHEAT_HOTEND_TEMP))
                : PSTR("PVB  -  " STRINGIFY(PVB_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PVB_PREHEAT_HPB_TEMP)),
            mFilamentItem_PVB);
        MENU_ITEM_SUBMENU_P(bPreheatOnlyNozzle
                                ? PSTR("PA   -  " STRINGIFY(PA_PREHEAT_HOTEND_TEMP))
                                : PSTR("PA   -  " STRINGIFY(PA_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PA_PREHEAT_HPB_TEMP)),
                            mFilamentItem_PA);
        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("ABS  -  " STRINGIFY(ABS_PREHEAT_HOTEND_TEMP))
                : PSTR("ABS  -  " STRINGIFY(ABS_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(ABS_PREHEAT_HPB_TEMP)),
            mFilamentItem_ABS);
        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("HIPS -  " STRINGIFY(HIPS_PREHEAT_HOTEND_TEMP))
                : PSTR("HIPS -  " STRINGIFY(HIPS_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(HIPS_PREHEAT_HPB_TEMP)),
            mFilamentItem_HIPS);
        MENU_ITEM_SUBMENU_P(bPreheatOnlyNozzle
                                ? PSTR("PP   -  " STRINGIFY(PP_PREHEAT_HOTEND_TEMP))
                                : PSTR("PP   -  " STRINGIFY(PP_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(PP_PREHEAT_HPB_TEMP)),
                            mFilamentItem_PP);
        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("FLEX -  " STRINGIFY(FLEX_PREHEAT_HOTEND_TEMP))
                : PSTR("FLEX -  " STRINGIFY(FLEX_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(FLEX_PREHEAT_HPB_TEMP)),
            mFilamentItem_FLEX);

        MENU_ITEM_SUBMENU_P(
            bPreheatOnlyNozzle
                ? PSTR("Clean  -  " STRINGIFY(CLEAN_PREHEAT_HOTEND_TEMP))
                : PSTR("Clean  -  " STRINGIFY(CLEAN_PREHEAT_HOTEND_TEMP) "/" STRINGIFY(CLEAN_PREHEAT_HPB_TEMP)),
            mFilamentItem_Clean);
    }
...
static void lcd_unLoadFilament() { preheat_or_continue(FilamentAction::UnLoad); }
...
void lcd_loading_color() {
    ...
    lcd_loading_progress_bar((FILAMENTCHANGE_FINALFEED * 1000ul) /
                             FILAMENTCHANGE_EFEED_FINAL);  // show progress bar during filament loading slow sequence
}
...
void lcd_loading_filament() {
    ...
    lcd_loading_progress_bar(
        slow_seq_time + fast_seq_time);  // show progress bar for total time of filament loading fast + slow sequence
}
...
void lcd_load_filament_color_check() {
    uint8_t clean =
        lcd_show_fullscreen_message_yes_no_and_wait_P(_T(MSG_FILAMENT_CLEAN), false, LCD_LEFT_BUTTON_CHOICE);
...
static void lcd_menu_AutoLoadFilament() {
    lcd_display_message_fullscreen_nonBlocking_P(
        _i("Autoloading filament is active, just press the knob and insert filament..."));  ////MSG_AUTOLOADING_ENABLED
                                                                                            /// c=20 r=4
    menu_back_if_clicked();
}
#endif  // FILAMENT_SENSOR
...
static void lcd_LoadFilament() { preheat_or_continue(FilamentAction::Load); }

void lcd_AutoLoadFilament() { preheat_or_continue(FilamentAction::AutoLoad); }
...
static void lcd_menu_xyz_y_min() {
    ...
    if (lcd_clicked()) menu_goto(lcd_menu_xyz_skew, 0, true);
}

//@brief Show measured axis skewness
float _deg(float rad) { return rad * 180 / M_PI; }
...
static void lcd_menu_xyz_skew() {
    ...
    lcd_home();
    lcd_printf_P(_N("%-14.14S:\n"
                    "%S\n"
                    "%-14.14S:%3.2f" LCD_STR_DEGREE "\n"
                    "%-14.14S:%3.2f" LCD_STR_DEGREE),
                 _i("Measured skew"),                                          ////MSG_MEASURED_SKEW c=14
                 STR_SEPARATOR, _i("Slight skew"), _deg(bed_skew_angle_mild),  ////MSG_SLIGHT_SKEW c=14
    ...
    if (lcd_clicked()) menu_goto(lcd_menu_xyz_offset, 0, true);
}
...
static void lcd_babystep_z() {
    ...
    if (_md->status == 0) {
        ...
        if (!eeprom_is_sheet_initialized(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))) {
            ...
            _md->babystepMemZ = eeprom_read_word(reinterpret_cast<uint16_t *>(
                &(EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].z_offset)));
        }

        // same logic as in babystep_load
        if (!calibration_status_get(CALIBRATION_STATUS_LIVE_ADJUST)) _md->babystepMemZ = 0;
    ...
    if (LCD_CLICKED || menu_leaving) {
        ...
        eeprom_update_word(reinterpret_cast<uint16_t *>(&(EEPROM_Sheets_base->s[active_sheet].z_offset)),
                           _md->babystepMemZ);
...
typedef struct {                    // 12bytes + 9bytes = 21bytes total
...
static_assert(sizeof(menu_data) >= sizeof(_menu_data_adjust_bed_t),
              "_menu_data_adjust_bed_t doesn't fit into menu_data");
...
void lcd_adjust_bed(void) {
    ...
    ON_MENU_LEAVE(eeprom_update_byte((uint8_t *)EEPROM_BED_CORRECTION_LEFT, (uint8_t)_md->left);
    ...
    MENU_ITEM_EDIT_int3_P(_i("Left side [\xe4m]"), &_md->left, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);  ////MSG_BED_CORRECTION_LEFT c=14
    MENU_ITEM_EDIT_int3_P(_i("Right side[\xe4m]"), &_md->right, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);  ////MSG_BED_CORRECTION_RIGHT c=14
    MENU_ITEM_EDIT_int3_P(_i("Front side[\xe4m]"), &_md->front, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);  ////MSG_BED_CORRECTION_FRONT c=14
    MENU_ITEM_EDIT_int3_P(_i("Rear side [\xe4m]"), &_md->rear, -BED_ADJUSTMENT_UM_MAX,
                          BED_ADJUSTMENT_UM_MAX);  ////MSG_BED_CORRECTION_REAR c=14
...
void pid_extruder() {
    ...
    else if (pid_temp < HEATER_0_MINTEMP)
...
bool lcd_calibrate_z_end_stop_manual(bool only_z) {
    ...
    for (;;) {
        const char *msg = only_z ? _i("Calibrating Z. Rotate the knob to move the Z carriage up to the end stoppers. "
                                      "Click when done.")  ////MSG_MOVE_CARRIAGE_TO_THE_TOP_Z c=20 r=8
                                 : _i("Calibrating XYZ. Rotate the knob to move the Z carriage up to the end stoppers. "
                                      "Click when done.");  ////MSG_MOVE_CARRIAGE_TO_THE_TOP c=20 r=8
        ...
        for (;;) {
            ...
            if (multi_screen && _millis() - previous_millis_msg > 5000) {
                if (msg_next == NULL) msg_next = msg;
                msg_next = lcd_display_message_fullscreen_P(msg_next);
                previous_millis_msg = _millis();
            }
        }
        // Let the user confirm, that the Z carriage is at the top end stoppers.
        uint8_t result = lcd_show_fullscreen_message_yes_no_and_wait_P(
            _i("Are left and right Z-carriages all up?"), false);  ////MSG_CONFIRM_CARRIAGE_AT_THE_TOP c=20 r=3
...
void lcd_wait_for_click() { lcd_wait_for_click_delay(0); }
...
uint8_t lcd_show_multiscreen_message_yes_no_and_wait_P(
    const char *msg, bool allow_timeouting,
    uint8_t default_selection)  // currently just max. n*4 + 3 lines supported (set in language header files)
{
    return lcd_show_multiscreen_message_with_choices_and_wait_P(msg, allow_timeouting, default_selection, _T(MSG_YES),
                                                                _T(MSG_NO), nullptr, 10);
}
...
void lcd_show_choices_prompt_P(uint8_t selected, const char *first_choice, const char *second_choice,
                               uint8_t second_col, const char *third_choice) {
...
//! @param default_selection uint8_t, Control which choice is selected first. 0: left most, 1: middle, 2: right most
//! choice. The left most choice is selected by default
...
//! @param third_choice text caption of second possible choice. Must be in PROGMEM. When not set to nullptr first_choice
//! and second_choice may not be more than 5 characters long.
...
//! @retval 0xFF button timeout (only possible if allow_timeouting is true)
uint8_t lcd_show_multiscreen_message_with_choices_and_wait_P(const char *const msg, bool allow_timeouting,
                                                             uint8_t default_selection, const char *const first_choice,
                                                             const char *const second_choice,
                                                             const char *const third_choice, uint8_t second_col) {
...
exit:
    ...
    // because it may create a recursion scenario when the caller of
    // lcd_show_multiscreen_message_with_choices_and_wait_P is a submenu lcd_update_enable(true) will cause another call
    // to the submenu immediately and so won't allow the user to exit the submenu
    lcd_update_enabled = true;
    lcd_draw_update = 2;
...
uint8_t lcd_show_fullscreen_message_yes_no_and_wait_P(const char *msg, bool allow_timeouting,
...
void lcd_bed_calibration_show_result(BedSkewOffsetDetectionResultType result, uint8_t point_too_far_mask) {
    ...
    if (result == BED_SKEW_OFFSET_DETECTION_POINT_NOT_FOUND) {
        lcd_show_fullscreen_message_and_wait_P(_i(
            "XYZ calibration failed. Bed calibration point was not found."));  ////MSG_BED_SKEW_OFFSET_DETECTION_POINT_NOT_FOUND
                                                                               /// c=20 r=6
        ...
        if (point_too_far_mask == 0)
            ...
            msg = _i(
                "XYZ calibration failed. Front calibration points not reachable.");  ////MSG_BED_SKEW_OFFSET_DETECTION_FAILED_FRONT_BOTH_FAR
                                                                                     /// c=20 r=6
        else if ((point_too_far_mask & 1) == 0)
            // The right and maybe the center point out of reach.
            msg = _i(
                "XYZ calibration failed. Right front calibration point not reachable.");  ////MSG_BED_SKEW_OFFSET_DETECTION_FAILED_FRONT_RIGHT_FAR
                                                                                          /// c=20 r=6
        else
            // The left and maybe the center point out of reach. //@todo Why isn't it found in the firmware.map
            msg = _n(
                "XYZ calibration failed. Left front calibration point not reachable.");  ////MSG_BED_SKEW_OFFSET_DETECTION_FAILED_FRONT_LEFT_FAR
                                                                                         /// c=20 r=8
        ...
        if (point_too_far_mask != 0) {
            if (point_too_far_mask == 2 || point_too_far_mask == 7)
                // Only the center point or all the three front points.
                msg = _i(
                    "XYZ calibration compromised. Front calibration points not reachable.");  ////MSG_BED_SKEW_OFFSET_DETECTION_WARNING_FRONT_BOTH_FAR
                                                                                              /// c=20 r=8
            else if ((point_too_far_mask & 1) == 0)
                // The right and maybe the center point out of reach.
                msg = _i(
                    "XYZ calibration compromised. Right front calibration point not reachable.");  ////MSG_BED_SKEW_OFFSET_DETECTION_WARNING_FRONT_RIGHT_FAR
                                                                                                   /// c=20 r=8
            else
                // The left and maybe the center point out of reach. //@todo Why isn't it found in the firmware.map
                msg = _n(
                    "XYZ calibration compromised. Left front calibration point not reachable.");  ////MSG_BED_SKEW_OFFSET_DETECTION_WARNING_FRONT_LEFT_FAR
                                                                                                  /// c=20 r=8
            lcd_show_fullscreen_message_and_wait_P(msg);
        }
        if (point_too_far_mask == 0 || result > 0) {
            switch (result) {
                ...
                case BED_SKEW_OFFSET_DETECTION_PERFECT:
                    msg = _i(
                        "XYZ calibration ok. X/Y axes are perpendicular. Congratulations!");  ////MSG_BED_SKEW_OFFSET_DETECTION_PERFECT
                                                                                              /// c=20 r=8
                    break;
                case BED_SKEW_OFFSET_DETECTION_SKEW_MILD:
                    msg = _i(
                        "XYZ calibration all right. X/Y axes are slightly skewed. Good job!");  ////MSG_BED_SKEW_OFFSET_DETECTION_SKEW_MILD
                                                                                                /// c=20 r=8
                    break;
                case BED_SKEW_OFFSET_DETECTION_SKEW_EXTREME:
                    msg = _i(
                        "XYZ calibration all right. Skew will be corrected automatically.");  ////MSG_BED_SKEW_OFFSET_DETECTION_SKEW_EXTREME
                                                                                              /// c=20 r=8
                    break;
            }
            lcd_show_fullscreen_message_and_wait_P(msg);
...
static void lcd_show_sensors_state() {
    ...
    lcd_printf_P(_N("B: %3d     Xd:%6d\n"
                    "S: %3d     Yd:%6d"),
                 pat9125_b, pat9125_x, pat9125_s, pat9125_y);
...
static void lcd_mmu_mode_toggle() { eeprom_toggle((uint8_t *)EEPROM_MMU_STEALTH); }
...
static void menu_setlang(uint8_t lang) {
    if (!lang_select(lang)) {
        if (lcd_show_fullscreen_message_yes_no_and_wait_P(_i("Copy selected language?"), false,
                                                          LCD_LEFT_BUTTON_CHOICE) ==
            LCD_LEFT_BUTTON_CHOICE)  ////MSG_COPY_SEL_LANG c=20 r=3
...
static void lcd_language_menu() {
    ...
    if (cnt == 2)  // display secondary language in case of clear xflash
    {
        ...
        for (uint8_t i = 2; i < 8;
             i++)  // skip seconday language - solved in lang_select (MK3) 'i < 8'  for 7 official languages
#else              // XFLASH
...
void lcd_first_layer_calibration_reset() {
    ...
    eeprom_read_block(sheet_name, &EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].name,
                      sizeof(Sheet::name));
    lcd_home();
    float offset = static_cast<int16_t>(eeprom_read_word(reinterpret_cast<uint16_t *>(
                       &EEPROM_Sheets_base->s[(eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet)))].z_offset))) /
                   cs.axis_steps_per_mm[Z_AXIS];
    lcd_printf_P(
        _i("Sheet %.7s\nZ offset: %+1.3fmm\n%cContinue\n%cReset"),  ////MSG_SHEET_OFFSET c=20 r=4
        sheet_name, offset, menuData->reset ? ' ' : '>',
        menuData->reset
            ? '>'
            : ' ');  // \n denotes line break, %.7s is replaced by 7 character long sheet name, %+1.3f is replaced by 6
                     // character long floating point number, %c is replaced by > or white space (one character) based
                     // on whether first or second option is selected. % denoted place holders can not be reordered.
}

void lcd_v2_calibration() {
    if (MMU2::mmu2.Enabled()) {
        const uint8_t filament = choose_menu_P(_T(MSG_SELECT_FILAMENT), _T(MSG_FILAMENT),
                                               (_T(MSG_CANCEL) + 1));  // Hack to reuse MSG but strip 1st char off
        ...
        if (fsensor.isReady()) {
            ...
            loaded = !lcd_show_fullscreen_message_yes_no_and_wait_P(_T(MSG_FILAMENT_LOADED), false,
...
void lcd_wizard() {
    ...
    if (calibration_status_get(CALIBRATION_WIZARD_STEPS)) {
        ...
        result = !lcd_show_multiscreen_message_yes_no_and_wait_P(
            _i("Running Wizard will delete current calibration results and start from the beginning. Continue?"),
            false);  ////MSG_WIZARD_RERUN c=20 r=7
    }
    if (result) {
...
static void lcd_wizard_load() {
    if (MMU2::mmu2.Enabled()) {
        ...
        lcd_show_fullscreen_message_and_wait_P(_i(
            "Please insert filament into the extruder, then press the knob to load it."));  ////MSG_WIZARD_LOAD_FILAMENT
                                                                                            /// c=20 r=6
        lcd_update_enable(false);
        lcd_clear();
...
static void wizard_lay1cal_message(bool cold) {
    lcd_show_fullscreen_message_and_wait_P(
        _i("Now I will calibrate distance between tip of the nozzle and heatbed surface."));  ////MSG_WIZARD_V2_CAL c=20
                                                                                              /// r=8
    if (MMU2::mmu2.Enabled()) {
        lcd_show_fullscreen_message_and_wait_P(_i(
            "Select a filament for the First Layer Calibration and select it in the on-screen menu."));  ////MSG_SELECT_FIL_1ST_LAYERCAL
                                                                                                         /// c=20 r=7
    } else if (cold) {
        lcd_show_fullscreen_message_and_wait_P(
    ...
    lcd_show_fullscreen_message_and_wait_P(
        _i("The printer will start printing a zig-zag line. Rotate the knob until you reach the optimal height. Check "
           "the pictures in the handbook (Calibration chapter)."));  ////MSG_WIZARD_V2_CAL_2 c=20 r=12
}
    ...
    while (!end) {
        ...
        switch (state) {
            case S::Run:  // Run wizard?
                ...
                // It is caused by the fact, that tmc2130_st_isr makes a crash detection before the printers really
                // starts. And thus it calles stop_and_save_print_to_ram which sets the saved_printing flag. Having this
                // flag set during normal printing is lethal - mesh_plan_buffer_line exist in the middle of planning
                // long travels which results in distorted print. This primarily happens when the printer is new and
                // parked in 0,0 So any new printer will fail the first layer calibration unless being reset or the Stop
                // function gets called. We really must find a way to prevent the crash from happening before the
                // printer is started - that would be the correct solution. Btw. the flag may even trigger the viper
                // situation on normal start this way and the user won't be able to find out why.
                ...
                if (eeprom_read_byte((uint8_t *)EEPROM_WIZARD_ACTIVE) == 2) {
                    ...
                    wizard_event = lcd_show_multiscreen_message_yes_no_and_wait_P(_T(MSG_WIZARD_WELCOME), false,
                                                                                  LCD_LEFT_BUTTON_CHOICE);
            ...
            case S::Selftest:
                lcd_show_fullscreen_message_and_wait_P(_i(
                    "First, I will run the selftest to check most common assembly problems."));  ////MSG_WIZARD_SELFTEST
                                                                                                 /// c=20 r=8
            ...
            case S::Xyz:
                lcd_show_fullscreen_message_and_wait_P(
                    _i("I will run xyz calibration now. It will take up to 24 mins."));  ////MSG_WIZARD_XYZ_CAL c=20 r=8
                wizard_event = gcode_M45(false, 0);
                state = (wizard_event ? S::Restore : S::Failed);
                break;
            case S::Z:
                lcd_show_fullscreen_message_and_wait_P(
                    _i("Please remove shipping helpers first."));  ////MSG_REMOVE_SHIPPING_HELPERS c=20 r=3
                lcd_show_fullscreen_message_and_wait_P(
                    _i("Now remove the test print from steel sheet."));  ////MSG_REMOVE_TEST_PRINT c=20 r=4
                ...
                if (!wizard_event) {
                    ...
                    lcd_display_message_fullscreen_P(
                        _i("Now I will preheat nozzle for PLA."));  ////MSG_WIZARD_WILL_PREHEAT c=20 r=4
                    wait_preheat();
            ...
            case S::IsFil:
                ...
                wizard_event = lcd_show_fullscreen_message_yes_no_and_wait_P(_T(MSG_FILAMENT_LOADED), false,
                                                                             LCD_MIDDLE_BUTTON_CHOICE);
                if (wizard_event == LCD_LEFT_BUTTON_CHOICE) {
                    state = S::Lay1CalCold;
            ...
            case S::Preheat:
                ...
                lcd_show_fullscreen_message_and_wait_P(
                    _i("Select nozzle preheat temperature which matches your material."));  ////MSG_SEL_PREHEAT_TEMP
                                                                                            /// c=20 r=6
            ...
            case S::RepeatLay1Cal:
                wizard_event = lcd_show_multiscreen_message_yes_no_and_wait_P(
                    _i("Do you want to repeat last step to readjust distance between nozzle and heatbed?"),
                    false);  ////MSG_WIZARD_REPEAT_V2_CAL c=20 r=7
                if (wizard_event == LCD_LEFT_BUTTON_CHOICE) {
                    lcd_show_fullscreen_message_and_wait_P(
                    ...
                    lcd_show_fullscreen_message_and_wait_P(
                        _i("If you have additional steel sheets, calibrate their presets in Settings - HW Setup - "
                           "Steel sheets."));  ////MSG_ADDITIONAL_SHEETS c=20 r=8
                    state = S::Restore;
                }
                break;
            case S::Finish:
...
void lcd_settings_linearity_correction_menu(void) {
    MENU_BEGIN();
    ON_MENU_LEAVE(lcd_settings_linearity_correction_menu_save(););
    MENU_ITEM_BACK_P(_T(MSG_SETTINGS));
#ifdef TMC2130_LINEARITY_CORRECTION_XYZ
    MENU_ITEM_EDIT_int3_P(_i("X-correct"), &tmc2130_wave_fac[X_AXIS],
                          TMC2130_WAVE_FAC1000_MIN - TMC2130_WAVE_FAC1000_STP,
                          TMC2130_WAVE_FAC1000_MAX);  ////MSG_X_CORRECTION c=13
    MENU_ITEM_EDIT_int3_P(_i("Y-correct"), &tmc2130_wave_fac[Y_AXIS],
                          TMC2130_WAVE_FAC1000_MIN - TMC2130_WAVE_FAC1000_STP,
                          TMC2130_WAVE_FAC1000_MAX);  ////MSG_Y_CORRECTION c=13
    MENU_ITEM_EDIT_int3_P(_i("Z-correct"), &tmc2130_wave_fac[Z_AXIS],
                          TMC2130_WAVE_FAC1000_MIN - TMC2130_WAVE_FAC1000_STP,
                          TMC2130_WAVE_FAC1000_MAX);  ////MSG_Z_CORRECTION c=13
#endif                                                // TMC2130_LINEARITY_CORRECTION_XYZ
    MENU_ITEM_EDIT_int3_P(_i("E-correct"), &tmc2130_wave_fac[E_AXIS],
                          TMC2130_WAVE_FAC1000_MIN - TMC2130_WAVE_FAC1000_STP,
                          TMC2130_WAVE_FAC1000_MAX);  ////MSG_EXTRUDER_CORRECTION c=13
    MENU_END();
}
#endif  // TMC2130
...
static void fsensor_reinit() { fsensor.init(); }

static void lcd_fsensor_enabled_set(void) { fsensor.setEnabled(!fsensor.isEnabled()); }

static void lcd_fsensor_runout_set() { fsensor.setRunoutEnabled(!fsensor.getRunoutEnabled(), true); }

static void lcd_fsensor_autoload_set() { fsensor.setAutoLoadEnabled(!fsensor.getAutoLoadEnabled(), true); }

#if FILAMENT_SENSOR_TYPE == FSENSOR_PAT9125
static void lcd_fsensor_jam_detection_set() { fsensor.setJamDetectionEnabled(!fsensor.getJamDetectionEnabled(), true); }
...
static void lcd_fsensor_settings_menu() {
    ...
    if (fsensor.isEnabled()) {
        if (fsensor.isError()) {
            ...
            MENU_ITEM_TOGGLE_P(_T(MSG_FSENSOR_RUNOUT), fsensor.getRunoutEnabled() ? _T(MSG_ON) : _T(MSG_OFF),
                               lcd_fsensor_runout_set);
            MENU_ITEM_TOGGLE_P(_T(MSG_FSENSOR_AUTOLOAD), fsensor.getAutoLoadEnabled() ? _T(MSG_ON) : _T(MSG_OFF),
                               lcd_fsensor_autoload_set);
#if defined(FILAMENT_SENSOR) && (FILAMENT_SENSOR_TYPE == FSENSOR_PAT9125)
            MENU_ITEM_TOGGLE_P(_T(MSG_FSENSOR_JAM_DETECTION),
                               fsensor.getJamDetectionEnabled() ? _T(MSG_ON) : _T(MSG_OFF),
                               lcd_fsensor_jam_detection_set);
#endif  // defined(FILAMENT_SENSOR) && (FILAMENT_SENSOR_TYPE == FSENSOR_PAT9125)
        }
...
static void menuitems_MMU_settings_common() {
    MENU_ITEM_TOGGLE_P(MSG_SPOOL_JOIN, SpoolJoin::spooljoin.isSpoolJoinEnabled() ? _T(MSG_ON) : _T(MSG_OFF),
                       SpoolJoin::spooljoin.toggleSpoolJoin);
...
#ifndef MMU_FORCE_STEALTH_MODE
    MENU_ITEM_TOGGLE_P(_T(MSG_MMU_MODE),
                       eeprom_read_byte((uint8_t *)EEPROM_MMU_STEALTH) ? _T(MSG_STEALTH) : _T(MSG_NORMAL),
                       lcd_mmu_mode_toggle);
#endif  // MMU_FORCE_STEALTH_MODE
}
...
static void SETTINGS_SILENT_MODE() {
    if (!farm_mode) {  // dont show in menu if we are in farm mode
        ...
        if (eeprom_mode == SILENT_MODE_NORMAL) {
            ...
            MENU_ITEM_TOGGLE_P(_T(MSG_CRASHDETECT),
                               eeprom_read_byte((uint8_t *)EEPROM_CRASH_DET) ? _T(MSG_ON) : _T(MSG_OFF),
                               crash_mode_switch);
        } else {
            if (bDesync) {
...
#define SETTINGS_NOZZLE                                                                               \
    do {                                                                                              \
        switch (oNozzleDiameter) {                                                                    \
            case ClNozzleDiameter::_Diameter_250:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.25"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_300:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.30"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_Undef:                                                   \
            case ClNozzleDiameter::_Diameter_400:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.40"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_500:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.50"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_600:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.60"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_700:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.70"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_800:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.80"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_900:                                                     \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("0.90"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
            case ClNozzleDiameter::_Diameter_1000:                                                    \
                MENU_ITEM_TOGGLE_P(_T(MSG_NOZZLE_DIAMETER), PSTR("1.00"), lcd_nozzle_diameter_cycle); \
                break;                                                                                \
        }                                                                                             \
    } while (0)
...
void lcd_hw_setup_menu(void)  // can not be "static"
{
    ...
    MENU_BEGIN();
    MENU_ITEM_BACK_P(
        _T(bSettings ? MSG_SETTINGS : MSG_BACK));  // i.e. default menu-item / menu-item after checking mismatch
    ...
    MENU_ITEM_TOGGLE_P(_N("SuperPINDA"),
                       eeprom_read_byte((uint8_t *)EEPROM_PINDA_TEMP_COMPENSATION) ? _T(MSG_YES) : _T(MSG_NO),
                       lcd_pinda_temp_compensation_toggle);
...
static void lcd_settings_menu() {
    ...
    MENU_ITEM_TOGGLE_P(PSTR("MMU"), eeprom_read_byte((uint8_t *)EEPROM_MMU_ENABLED) ? _T(MSG_ON) : _T(MSG_OFF),
                       mmu_enable_switch);
    if (eeprom_read_byte((uint8_t *)EEPROM_MMU_ENABLED)) {  // Communication with MMU not required to reset MMU
        MENU_ITEM_FUNCTION_P(PSTR("Reset MMU"), []() { MMU2::mmu2.Reset(MMU2::MMU2::ResetForm::Software); });
    }

    if (MMU2::mmu2.Enabled()) {  // Only show menus when communicating with MMU
    ...
    if (has_temperature_compensation())
        MENU_ITEM_TOGGLE_P(_T(MSG_PINDA_CALIBRATION),
                           eeprom_read_byte((unsigned char *)EEPROM_TEMP_CAL_ACTIVE) ? _T(MSG_ON) : _T(MSG_OFF),
    ...
    if (!farm_mode) {  // SD related settings are not available in farm mode
        if (card.ToshibaFlashAir_isEnabled())
            MENU_ITEM_TOGGLE_P(_T(MSG_SD_CARD), MSG_TOSHIBA_FLASH_AIR_COMPATIBILITY,
    ...
    MENU_ITEM_TOGGLE_P(_N("HBed on load"),
                       eeprom_read_byte((uint8_t *)EEPROM_HEAT_BED_ON_LOAD_FILAMENT) ? _T(MSG_YES) : _T(MSG_NO),
                       lcd_heat_bed_on_load_toggle);  ////MSG_HEAT_BED_ON_LOAD c=12
...
#ifdef PINDA_THERMISTOR
    if (has_temperature_compensation()) MENU_ITEM_FUNCTION_P(_T(MSG_PINDA_CALIBRATION), lcd_calibrate_pinda);
#endif
...
char reset_menu() {
    ...
    const char *const item[] = {
        PSTR("Language"), PSTR("Statistics"), PSTR("Shipping prep"), PSTR("Service prep"), PSTR("All Data"),
    };
...
static void lcd_disable_farm_mode() {
    uint8_t disable = lcd_show_fullscreen_message_yes_no_and_wait_P(PSTR("Disable farm mode?"),
                                                                    true);  // allow timeouting, default no
...
static inline void load_filament_wrapper(uint8_t i) { MMU2::mmu2.load_filament(i); }
...
static void mmu_common_choose_filament_menu(const char *label, void (*menuAction)(uint8_t)) {
    ...
    ON_MENU_LEAVE(clearFilamentAction(););
    MENU_ITEM_BACK_P(_T(MSG_MAIN));
    for (uint8_t i = 0; i < MMU_FILAMENT_COUNT; i++) MENU_ITEM_FUNCTION_NR_P(label, i + '1', menuAction, i);
    MENU_END();
}
...
#ifdef MMU_HAS_CUTTER
static inline void mmu_cut_filament_wrapper(uint8_t index) { MMU2::mmu2.cut_filament(index); }

static void mmu_cut_filament_menu() { mmu_common_choose_filament_menu(_T(MSG_CUT_FILAMENT), mmu_cut_filament_wrapper); }
#endif  // MMU_HAS_CUTTER
...
static void lcd_mmuLoadingTest() { preheat_or_continue(FilamentAction::MmuLoadingTest); }

static void lcd_mmuCutFilament() { preheat_or_continue(FilamentAction::MmuCut); }

static void lcd_mmuLoadFilament() { preheat_or_continue(FilamentAction::MmuLoad); }

static void lcd_mmuUnloadFilament() { preheat_or_continue(FilamentAction::MmuUnLoad); }

static void lcd_mmuEjectFilament() { preheat_or_continue(FilamentAction::MmuEject); }
...
static void lcd_reset_sheet() {
    ...
    eeprom_update_word(reinterpret_cast<uint16_t *>(&(EEPROM_Sheets_base->s[selected_sheet].z_offset)),
                       EEPROM_EMPTY_VALUE16);
    ...
    if (selected_sheet == eeprom_read_byte(&(EEPROM_Sheets_base->active_sheet))) {
        ...
        if (-1 == eeprom_next_initialized_sheet(0)) calibration_status_clear(CALIBRATION_STATUS_LIVE_ADJUST);
    }
...
static void lcd_main_menu() {
    ...
    if (farm_mode) MENU_ITEM_FUNCTION_P(_T(MSG_FILAMENTCHANGE), lcd_colorprint_change);  // 8
    ...
    if ((printJobOngoing() || printingIsPaused()) && (custom_message_type != CustomMsg::MeshBedLeveling) &&
        !processing_tcode) {
    ...
    if (!get_temp_error()
        ...
        if (card.mounted || lcd_commands_type != LcdCommands::Idle) {
...
#if SDCARDDETECT < 1
                MENU_ITEM_GCODE_P(_i("Change SD card"),
                                  PSTR("M21"));  // SD-card changed by user ////MSG_CNG_SDCARD c=18
#endif                                           // SDCARDDETECT
            }
        } else {
            ...
            MENU_ITEM_GCODE_P(
                _i("Init. SD card"),
                PSTR("M21"));  // Manually initialize the SD-card via user interface ////MSG_INIT_SDCARD c=18
#endif                         // SDCARDDETECT
        }
#endif  // SDSUPPORT
    }
...
void stepper_timer_overflow() {
    char msg[28];
    sprintf_P(msg, PSTR("#%d %d max %d"), ++stepper_timer_overflow_cnt, stepper_timer_overflow_last >> 1,
              stepper_timer_overflow_max >> 1);
    lcd_setstatus(msg);
...
static void lcd_tune_menu() {
    ...
    MENU_BEGIN();
    ON_MENU_LEAVE(refresh_saved_feedrate_multiplier_in_ram(););
    MENU_ITEM_BACK_P(_T(MSG_MAIN));
    ...
    if (!farm_mode) MENU_ITEM_FUNCTION_P(_T(MSG_FILAMENTCHANGE), lcd_colorprint_change);
#endif
    if (printingIsPaused()) {  // Don't allow rehome if actively printing. Maaaaybe it could work to insert on the fly,
                               // seems too risky.
...
static void lcd_mesh_bed_leveling_settings() {
    ...
    MENU_ITEM_TOGGLE_P(_T(MSG_MAGNETS_COMP),
                       (points_nr == 7) ? (magnet_elimination ? _T(MSG_ON) : _T(MSG_OFF)) : _T(MSG_NA),
                       mbl_magnets_elimination_toggle);
    MENU_END();
}
...
static void lcd_backlight_menu() {
    ...
    MENU_ITEM_TOGGLE_P(_T(MSG_MODE),
                       ((backlightMode == BACKLIGHT_MODE_BRIGHT)
                            ? _T(MSG_BRIGHT)
                            : ((backlightMode == BACKLIGHT_MODE_DIM) ? _T(MSG_DIM) : _T(MSG_AUTO))),
                       backlight_mode_toggle);
...
void lcd_print_stop() { print_stop(true); }
...
void lcd_sdcard_menu() {
    ...
    switch (_md->menuState) {
        case _uninitialized:  // Initialize menu data
        {
            ...
            _md->row = -1;  // assume that no SD file/dir is currently selected. Once they are rendered, it will be
                            // changed to the correct row for the _scrolling state.
        }
        // FALLTHRU
        case _standard:  // normal menu structure.
        {
            if (!_md->lcd_scrollTimer.running())  // if the timer is not running, then the menu state was just switched,
                                                  // so redraw the screen.
            {
                _md->lcd_scrollTimer.start();
                lcd_draw_update = 1;
            }
            if ((lcd_draw_update == 0) && _md->lcd_scrollTimer.expired(500) &&
                (_md->row != -1))  // switch to the scrolling state on timeout if a file/dir is selected.
            {
            ...
            // if we reached this point it means that the encoder moved or clicked or the state is being switched. Reset
            // the scrollTimer.
            _md->lcd_scrollTimer.start();

            MENU_BEGIN();
            MENU_ITEM_BACK_P(
                _T(bMain ? MSG_MAIN : MSG_BACK));  // i.e. default menu-item / menu-item after card insertion
            card.getWorkDirName();
            if (card.filename[0] == '/') {
#if SDCARDDETECT == -1
...
#else
                    if (card.ToshibaFlashAir_isEnabled())
                        MENU_ITEM_FUNCTION_P(_T(MSG_REFRESH),
                                             lcd_sd_refresh);  // show the refresh option if in flashAir mode.
#endif
            } else
        ...
        case _scrolling:  // scrolling filename
        {
            ...
            const bool rewindFlag =
                LCD_CLICKED || lcd_draw_update;  // flag that says whether the menu should return to _standard state.
            ...
            if (rewindFlag) _md->offset = 0;  // redraw once again from the beginning.
...
void lcd_belttest() {
    ...
    // that clobbers ours, with more info than we could provide. So on fail we just fall through to take us back to
    // status.
...
static bool lcd_selftest_IRsensor(bool bStandalone) {
    ...
    newSensorRevision = (volt_IR_int < fsensor.IRsensor_Hopen_TRESHOLD) ? IR_sensor_analog::SensorRevision::_Rev04
                                                                        : IR_sensor_analog::SensorRevision::_Old;
    printf_P(PSTR("Measured filament sensor high level: %4.2fV\n"), Raw2Voltage(volt_IR_int));
    if (volt_IR_int < fsensor.IRsensor_Hmin_TRESHOLD) {
        if (!bStandalone) lcd_selftest_error(TestError::FsensorLevel, "HIGH", "");
        return false;
    }
    lcd_show_fullscreen_message_and_wait_P(
        _i("Insert the filament (do not load it) into the extruder and then press the knob."));  ////MSG_INSERT_FIL c=20
                                                                                                 /// r=6
    volt_IR_int = fsensor.getVoltRaw();
    printf_P(PSTR("Measured filament sensor low level: %4.2fV\n"), Raw2Voltage(volt_IR_int));
    if (volt_IR_int > (fsensor.IRsensor_Lmax_TRESHOLD)) {
        if (!bStandalone) lcd_selftest_error(TestError::FsensorLevel, "LOW", "");
        return false;
    }
...
static void lcd_detect_IRsensor() {
    ...
    if (loaded) {
        lcd_show_fullscreen_message_and_wait_P(
            _i("Please unload the filament first, then repeat this action."));  ////MSG_UNLOAD_FILAMENT_REPEAT c=20 r=4
        return;
    } else {
        lcd_show_fullscreen_message_and_wait_P(
            _i("Please check the IR sensor connection, unload filament if present."));  ////MSG_CHECK_IR_CONNECTION c=20
                                                                                        /// r=4
        bAction = lcd_selftest_IRsensor(true);
    }
    if (bAction) {
        lcd_show_fullscreen_message_and_wait_P(
            _i("Sensor verified, remove the filament now."));  ////MSG_FS_VERIFIED c=20 r=3
        fsensor.init();
    } else {
        lcd_show_fullscreen_message_and_wait_P(
            _i("Verification failed, remove the filament and try again."));  ////MSG_FIL_FAILED c=20 r=4
    }
}
#endif  // defined(FILAMENT_SENSOR) && (FILAMENT_SENSOR_TYPE == FSENSOR_IR_ANALOG)

static void lcd_selftest_v() { (void)lcd_selftest(); }
...
bool lcd_selftest() {
    ...
    if (_result) {
        // current_position[Z_AXIS] += 15;									//move Z
        // axis higher to avoid false triggering of Z end stop in case that we are very low - just above heatbed
    ...
    if (_result) {
        ...
        if (bres) eeprom_update_byte((uint8_t *)EEPROM_TMC2130_HOME_ENABLED, 1);
        _result = bres;
    }
#endif  // TMC2130
...
static bool lcd_selfcheck_axis_sg(uint8_t axis) {
    ...
    switch (axis) {
        case 0:
            ...
            break;
        case 1:
            axis_length = Y_MAX_POS - Y_MIN_POS + 4;
            break;
        default:
            axis_length = 210;
            break;
    ...
    if (fabs(measured_axis_length[0] - measured_axis_length[1]) >
        1) {  // check if difference between first and second measurement is low
        // loose pulleys
        const char *_error_1;
...
static bool lcd_selfcheck_axis(int _axis, int _travel) {
    ...
    do {
        ...
        plan_buffer_line_curposXYZE(manual_feedrate[0] / 60);
        st_synchronize();
        if ((READ(X_MIN_PIN) ^ (bool)X_MIN_ENDSTOP_INVERTING) || (READ(Y_MIN_PIN) ^ (bool)Y_MIN_ENDSTOP_INVERTING) ||
        ...
        if (_lcd_refresh < 6) {
            ...
            _progress = lcd_selftest_screen(static_cast<TestScreen>(static_cast<int>(TestScreen::AxisX) + _axis),
                                            _progress, 3, false, 0);
            _lcd_refresh = 0;
        }
...
static bool lcd_selfcheck_pulleys(int axis) {
    ...
    while (!endstop_triggered) {
        if (((READ(X_MIN_PIN) ^ X_MIN_ENDSTOP_INVERTING) == 1) || ((READ(Y_MIN_PIN) ^ Y_MIN_ENDSTOP_INVERTING) == 1)) {
            ...
            if (current_position_init - 1 <= current_position[axis] &&
                current_position_init + 1 >= current_position[axis]) {
                current_position[axis] += 10;
                plan_buffer_line_curposXYZE(manual_feedrate[0] / 60);
                st_synchronize();
                return (true);
...
static bool lcd_selfcheck_endstops() {
    ...
    if (
#ifndef TMC2130
        ((READ(X_MIN_PIN) ^ X_MIN_ENDSTOP_INVERTING) == 1) || ((READ(Y_MIN_PIN) ^ Y_MIN_ENDSTOP_INVERTING) == 1) ||
#endif  //! TMC2130
...
static bool lcd_selfcheck_check_heater(bool _isbed) {
    ...
    for (uint8_t _counter = 0; _counter < _cycles && !Stopped; ++_counter) {
        manage_heater();
        manage_inactivity(true);
        _progress = (_isbed ? lcd_selftest_screen(TestScreen::Bed, _progress, 2, false, 400)
...
static bool lcd_selftest_manual_fan_check(const uint8_t _fan, const bool check_opposite, const bool _default) {
...
static FanCheck lcd_selftest_fan_auto(uint8_t _fan) {
    ...
    static const int failThr = FANCHECK_AUTO_FAIL_THRS;  // < FANCHECK_AUTO_FAIL_THRS RPM would mean either a faulty
                                                         // Noctua, Altfan or print fan
...
static uint8_t lcd_selftest_screen(TestScreen screen, uint8_t _progress, uint8_t _progress_scale, bool _clear,
    ...
    if ((screen >= TestScreen::ExtruderFan) && (screen <= TestScreen::FansOk)) {
        ...
        lcd_selftest_screen_step(2, 2,
                                 ((screen == _step_block)  ? 1
                                  : (screen < _step_block) ? 0
                                                           : 2),
                                 PSTR("X"), _indicator);

        _step_block = TestScreen::AxisY;
        lcd_selftest_screen_step(2, 8,
                                 ((screen == _step_block)  ? 1
                                  : (screen < _step_block) ? 0
                                                           : 2),
                                 PSTR("Y"), _indicator);
...
static void lcd_selftest_screen_step(const uint8_t _row, const uint8_t _col, const uint8_t _state,
                                     const char *_name_PROGMEM, const char _indicator) {
...
static void menu_action_sdfile(const char *filename) {
    ...
    for (uint_least8_t i = 0; i < 8; i++) {
        if (selected_filename[i] == '\0' || selected_filename[i] == '.') {
            // filename is shorter then 8.3, store '\0' character on position where ".gco" string was found to terminate
            // stored string properly
    ...
    if (!check_file(selected_filename)) {
        result = !lcd_show_fullscreen_message_yes_no_and_wait_P(_i("File incomplete. Continue anyway?"),
                                                                false);  ////MSG_FILE_INCOMPLETE c=20 r=3
        lcd_update_enable(true);
    }
    if (result) {
...
static bool lcd_message_check(uint8_t priority) {
    ...
    if (priority >= lcd_status_message_level) return true;
...
void lcd_setstatus(const char *message) {
    if (lcd_message_check(LCD_STATUS_NONE)) lcd_updatestatus(message);
}
...
void lcd_getstatus(char buf[LCD_WIDTH]) { strncpy(buf, lcd_status_message, LCD_WIDTH); }

void lcd_setstatuspgm(const char *message) {
    if (lcd_message_check(LCD_STATUS_NONE)) lcd_updatestatus(message, true);
}
...
void lcd_reset_status_message_timeout() { lcd_status_message_timeout.start(); }

void lcd_setalertstatus_(const char *message, uint8_t severity, bool progmem) {
    if (lcd_message_check(severity)) {
        bool same = !(progmem ? strcmp_P(lcd_status_message, message) : strcmp(lcd_status_message, message));
...
void lcd_setalertstatus(const char *message, uint8_t severity) { lcd_setalertstatus_(message, severity, false); }

void lcd_setalertstatuspgm(const char *message, uint8_t severity) { lcd_setalertstatus_(message, severity, true); }

void lcd_reset_alert_level() { lcd_status_message_level = 0; }

uint8_t get_message_level() { return lcd_status_message_level; }
...
void menu_lcd_longpress_func(void) {
    ...
    if (blocks_queued() || printJobOngoing()) {  // long press as live-adj-z
        ...
        if (menu_menu == lcd_status_screen || menu_menu == lcd_main_menu || menu_menu == lcd_preheat_menu ||
            menu_menu == lcd_sdcard_menu || menu_menu == lcd_settings_menu || menu_menu == lcd_control_temperature_menu
#if (LANG_MODE != 0)
...
static inline bool z_menu_expired() {
    return (menu_menu == lcd_babystep_z &&
            (!babystep_allowed() || (lcd_commands_type == LcdCommands::Idle &&
                                     lcd_timeoutToStatus.expired(LCD_TIMEOUT_TO_STATUS_BABYSTEP_Z))));
}

static inline bool other_menu_expired() {
    return (menu_menu != lcd_status_screen && menu_menu != lcd_babystep_z &&
...
void menu_lcd_lcdupdate_func(void) {
    ...
    if ((IS_SD_INSERTED != lcd_oldcardstatus)) {
        ...
        if (lcd_oldcardstatus) {
            if (!card.mounted) {
                card.mount(false);  // delay the sorting to the sd menu. Otherwise, removing the SD card while sorting
                                    // will not menu_back()
...
void UserECool_toggle() {
    // this is only called when the experimental menu is visible, thus the first condition for enabling of the ECool
    // mode is met in this place The condition is intentionally inverted as we are toggling the state (i.e. if it was
    // enabled, we are disabling the feature and vice versa)
    ...
    // @@TODO I don't like this - disabling the experimental menu shall disable ECool mode, but it will not reinit the
    // TMC and I don't want to add more code for this experimental feature ... ideally do not reinit the TMC here at all
    // and let the user reset the printer.
...
bool FarmOrUserECool() { return farm_mode || UserECoolEnabled(); }
...
void lcd_experimental_menu() {
    ...
    MENU_ITEM_TOGGLE_P(_N("ALTFAN det."), altfanOverride_get() ? _T(MSG_OFF) : _T(MSG_ON),
                       altfanOverride_toggle);  ////MSG_MENU_ALTFAN c=18
#endif                                          // EXTRUDER_ALTFAN_DETECT

#ifdef TMC2130
    MENU_ITEM_TOGGLE_P(_N("E-cool mode"), UserECoolEnabled() ? _T(MSG_ON) : _T(MSG_OFF),
                       UserECool_toggle);  ////MSG_MENU_ECOOL c=18
#endif
...
void lcd_pinda_temp_compensation_toggle() {
    ...
    if (pinda_temp_compensation ==
        EEPROM_EMPTY_VALUE)           // On MK2.5/S the EEPROM_EMPTY_VALUE will be set to 0 during eeprom_init.
