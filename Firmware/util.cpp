...
// Allocate the version string in the program memory. Otherwise the string lands either on the stack or in the global
// RAM.
...
const char *FW_PRUSA3D_MAGIC_STR_P() { return FW_PRUSA3D_MAGIC_STR; }
...
inline bool is_whitespace_or_nl(char c) { return c == ' ' || c == '\t' || c == '\n' || c == '\r'; }

inline bool is_whitespace_or_nl_or_eol(char c) { return c == 0 || c == ' ' || c == '\t' || c == '\n' || c == '\r'; }

inline bool is_digit(char c) { return c >= '0' && c <= '9'; }

const char *__attribute__((noinline)) Number(const char *str, uint16_t *v) {
    *v = 0;
    while (is_digit(*str)) {
        *v *= 10;
        *v += *str - '0';
    ...
    if (*str++ == '-') {
        switch (*str) {
            case 'A':  // expect ALPHA
                ...
                return Tag(str, STR_REVISION_ALPHA, sizeof(STR_REVISION_ALPHA) - 1, FIRMWARE_REVISION_ALPHA,
                           version + 3);
            case 'B':  // expect BETA
            ...
            default:
                return false;  // fail everything else
        }
    }
...
inline bool strncmp_PP(const char *p1, const char *p2, uint8_t n) {
    for (; n > 0; --n, ++p1, ++p2) {
        ...
        {
            if ((pgm_read_byte(p1) + 32) < pgm_read_byte(p2)) return -1;
            if ((pgm_read_byte(p1) + 32) > pgm_read_byte(p2)) return 1;
        } else if (pgm_read_byte(p1) == 0) {
            return 0;
        } else {  // p1 is lowercase
            if (pgm_read_byte(p1) < pgm_read_byte(p2)) return -1;
            if (pgm_read_byte(p1) > pgm_read_byte(p2)) return 1;
        }
    }
    return 0;
...
inline int8_t is_provided_version_newer(const char *version_string) {
    ...
    if (!parse_version(version_string, ver_gcode)) return -1;
...
bool show_upgrade_dialog_if_version_newer(const char *version_string) {
    if (oCheckVersion == ClCheckVersion::_None) return false;
    ...
    if (upgrade) {
        lcd_display_message_fullscreen_P(
            _i("New firmware version available:"));  ////MSG_NEW_FIRMWARE_AVAILABLE c=20 r=2
        lcd_puts_at_P(0, 2, PSTR(""));
        for (const char *c = version_string; !is_whitespace_or_nl_or_eol(*c); ++c) lcd_putc(*c);
...
void fCheckModeInit() {
    ...
    oNozzleDiameter = (ClNozzleDiameter)eeprom_init_default_byte((uint8_t *)EEPROM_NOZZLE_DIAMETER,
                                                                 (uint8_t)ClNozzleDiameter::_Diameter_400);
    ...
    oCheckVersion =
        (ClCheckVersion)eeprom_init_default_byte((uint8_t *)EEPROM_CHECK_VERSION, (uint8_t)ClCheckVersion::_Warn);
...
static void render_M862_warnings(const char *warning, const char *strict, uint8_t check) {
    if (check == 1) {  // Warning, stop print if user selects 'No'
        if (lcd_show_fullscreen_message_yes_no_and_wait_P(warning, true, LCD_LEFT_BUTTON_CHOICE) ==
...
void nozzle_diameter_check(uint16_t nDiameter) {
    ...
    if (oCheckMode == ClCheckMode::_None) return;
    nDiameter_um = eeprom_read_word((uint16_t *)EEPROM_NOZZLE_DIAMETER_uM);
    if (nDiameter == nDiameter_um) return;
    // SERIAL_ECHO_START;
    ...
    render_M862_warnings(_T(MSG_NOZZLE_DIFFERS_CONTINUE), _T(MSG_NOZZLE_DIFFERS_CANCELLED), (uint8_t)oCheckMode);
...
void printer_model_check(uint16_t nPrinterModel, uint16_t actualPrinterModel) {
    ...
    if (nPrinterModel == actualPrinterModel) return;
    // SERIAL_ECHO_START;
    ...
    render_M862_warnings(_T(MSG_GCODE_DIFF_PRINTER_CONTINUE), _T(MSG_GCODE_DIFF_PRINTER_CANCELLED),
                         (uint8_t)oCheckModel);
}
...
void fw_version_check(const char *pVersion) {
    if (oCheckVersion == ClCheckVersion::_None) return;
    ...
    nCompareValueResult += mCompareValue(aVersion[2], eeprom_read_word((uint16_t *)EEPROM_FIRMWARE_VERSION_REVISION))
                           << 2;
    nCompareValueResult += mCompareValue(aVersion[3], eeprom_read_word((uint16_t *)EEPROM_FIRMWARE_VERSION_FLAVOR));
    if (nCompareValueResult <= COMPARE_VALUE_EQUAL) return;
    ...
    */
    ...
    render_M862_warnings(_T(MSG_GCODE_NEWER_FIRMWARE_CONTINUE), _T(MSG_GCODE_NEWER_FIRMWARE_CANCELLED),
...
void gcode_level_check(uint16_t nGcodeLevel) {
    ...
    render_M862_warnings(_T(MSG_GCODE_DIFF_CONTINUE), _T(MSG_GCODE_DIFF_CANCELLED), (uint8_t)oCheckGcode);
}
...
void ip4_to_str(char *dest, uint8_t *IP) { sprintf_P(dest, PSTR("%u.%u.%u.%u"), IP[0], IP[1], IP[2], IP[3]); }
