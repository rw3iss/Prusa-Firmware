...
#define FW_VERSION_FULL \
    STR(FW_MAJOR) "." STR(FW_MINOR) "." STR(FW_REVISION) "-" STR(FW_FLAVOR) "" STR(FW_FLAVERSION) "+" STR(FW_COMMITNR)
#endif
...
// #define SLOW_PWM_HEATERS // PWM with very low frequency (roughly 0.125Hz=8s) and minimum state time of approximately
// 1s useful for heaters driven by a relay
...
#define Z_RAISE_BEFORE_HOMING \
    5  // (in mm) Raise Z before homing (G28) for Probe Clearance.
...
#define Z_RAISE_BETWEEN_PROBINGS \
    5  // How much the extruder will be raised when traveling from between next probing points
...
// The value is the delay to turn the servo off after powered on - depends on the servo speed; 300ms is good value, but
// you can try lower it.
...
#define DEFAULT_NOMINAL_FILAMENT_DIA \
    1.75  // Enter the diameter (in mm) of the filament generally used (3.0 mm or 1.75 mm). Used by the volumetric
          // extrusion.

// Try to maintain a minimum distance from the bed even when Z is
// unknown when doing the following operations
#define MIN_Z_FOR_LOAD 75     // lcd filament loading or autoload
#define MIN_Z_FOR_UNLOAD 75   // lcd filament unloading
#define MIN_Z_FOR_SWAP 50     // filament change (including M600)
#define MIN_Z_FOR_PREHEAT 25  // lcd preheat
